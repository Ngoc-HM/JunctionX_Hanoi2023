...
import '../../blocs/inside_transfer_bloc.dart';
...
class _Transfer extends State<Transfer> {
  List<Widget> _feature = [Inside(), ToBank(), ToCard()];
  List<Widget> _featureName = [
    Padding(
      padding: EdgeInsets.symmetric(horizontal: 12),
      child: Text('Nội bộ bank',
          style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
          textAlign: TextAlign.center),
    ),
    Padding(
      padding: EdgeInsets.symmetric(horizontal: 12),
      child: Text('Chuyển tiền vào STK',
          style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold),
          textAlign: TextAlign.center),
    ),
    Padding(
      padding: EdgeInsets.symmetric(horizontal: 12),
      child: Text('Chuyển tiền vào số thẻ',
          style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold),
          textAlign: TextAlign.center),
    ),
  ];
    ...
    return Wrap(
      ...
      children: [
        ...
        Center(
            child: ToggleButtons(
                constraints:
                    BoxConstraints(maxWidth: 120, minHeight: 50, maxHeight: 50),
                ...
                onPressed: (int index) {
                  setState(() {
                    _stateIndex = index;
                    for (int i = 0; i < _featureName.length; i++)
                      _isFeatureSelected[i] = i == index;
                  });
                },
                  ...
                  setState(() {
                    ...
                    for (var i = 0; i < allUser.length; i++) {
                      if (_receiverController.text ==
                          allUser[allUser.keys.elementAt(i)]!['name']) {
                        //TransferDontDestroyOnLoad.receiverID = allUser.keys.elementAt(i);
                      ...
                      if (_receiverController.text != "" ||
                        ...
                        showDialog(
                            ...
                            builder: (BuildContext context) => AlertDialog(
                                  title: Text("Lỗi"),
                                  content:
                                      const Text('Không có tài khoản đã nhập'),
                                  actions: <Widget>[
                                    TextButton(
                                      onPressed: () =>
